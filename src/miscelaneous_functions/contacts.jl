...
@testitem "residue_residue_distance" begin
    ...
    ats = read_pdb(Testing.namd2_pdb, "protein")
    ...
    @test d ≈ 5.6703672f0
    # testing with two water molecules that are far apart
    sim = Simulation(Testing.namd2_pdb, Testing.namd2_traj; step=5);
    residues = collect(eachresidue(atoms(sim)));
    w1 = residues[10299]; w2 = residues[4013];
    d = zeros(length(sim)) 
    for (iframe, frame) in enumerate(sim)
        p = positions(frame); uc = unitcell(frame)
        d[iframe] = residue_residue_distance(w1, w2; positions=p, unitcell=uc)
    end
    @test d ≈ [50.634259302086754, 48.555572344628175, 30.945124796003856, 51.71157515992379]
end
...
function residue_contacts(sim::Simulation, atoms; dmax=4.0, ref=1)
    inds = index.(atoms)
    if ref isa Integer
        if ref > length(sim)
