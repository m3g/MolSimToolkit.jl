...
julia> coordination_number(sim, protein, tmao; solvent_natomspermol=14, cutoff=3.0)
...
function coordination_number(
    ...
    @showprogress for (iframe, frame) in enumerate(sim)
        p = positions(frame)
...
@testitem "coordination_number" begin
    ...
    cn = coordination_number(sim, protein, tmao; solvent_natomspermol=14, cutoff=3.0)
