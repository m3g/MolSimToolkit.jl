...
julia> sum_of_dist = new_weigths(simulation, (i,j,d2) -> intermol_perturb(i, j, d2, dist), i1, i2, 25.0)
-------------
FRAME WEIGHTS
-------------
...
function new_weigths(simulation::Simulation, f_perturbation::Function, group_1::Vector{Int64}, cutoff::Float64, k::Float64 = 1.0, T::Float64 = 1.0)
    ...
    output = Resampling_results(prob_vec, prob_rel_vec, energy_vec)
    return output
end
...
function Base.show(io::IO, mime::MIME"text/plain", res::Resampling_results)
    print(
    io,
    """
    -------------
